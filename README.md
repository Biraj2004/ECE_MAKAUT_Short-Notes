...
| File | Topics |
|---|---|
| `PE-EC703B_Module1_Notes.pdf` | Introduction to Sensor Networks: definition, unique constraints (energy, computation, memory, communication), challenges (scalability, fault-tolerance, QoS), advantages, applications (military, environmental, health, industrial, home), types of WSN (terrestrial, underground, underwater, multimedia, mobile) |
| `PE-EC703B_Module2_Notes.pdf` | MANETs vs WSN (6-parameter comparison table); enabling technologies (MEMS, IEEE 802.15.4, low-power MCU, energy harvesting, TinyOS, 6LoWPAN); single-node architecture (sensing, processing, communication, power subsystems); hardware components; design constraints (energy, cost, size, memory, range, reliability) |
...
| File | Topics |
|---|---|
| `PE-EC703C_Module1_Notes.pdf` | Introduction and Historical Development: Definition, Father/Mother wavelet concept, time-frequency resolution limitations (FT, STFT, Heisenberg Uncertainty Principle, tiling grids), historical origin and different communities of wavelets, classification of continuous and discrete wavelet transforms, developments in wavelet theory applications. |
| `PE-EC703C_Module2_Notes.pdf` | Continuous Wavelet Transform: Continuous time wavelets, CWT definition, scale and translation parameters, constant Q-factor filtering interpretation and time-frequency resolution, linear operator, admissibility condition, Inverse CWT, Parseval's Relation. |
| `PE-EC703C_Module3_Notes.pdf` | Discrete Wavelet Transform and MRA: Approximations of vectors in nested linear vector subspaces, five MRA axioms, refinement/dilation equations, detail spaces, Haar scaling and wavelet functions, Haar wavelet decomposition, Haar wavelet packets and applications. |
| `PE-EC703C_Module4_Notes.pdf` | MRA Orthonormal Wavelets \& Filter Banks: 2-channel subband coder filter banks, decimation/interpolation, perfect reconstruction (PR) conditions (aliasing cancellation, no distortion), Quadrature Mirror Filters (QMF), Mallat FWT algorithm, Daubechies (dbN), Symlets (symN), and Coiflets (coifN) orthogonal families, cascade algorithm. |
| `PE-EC703C_Module5_Notes.pdf` | Bi-orthogonal Wavelets \& Multi-dimensional Wavelets: Dual scaling and wavelet functions, bi-orthogonal filter PR conditions, CDF 9/7 and 5/3 wavelets, separable 2D DWT subband quadrants (LL, LH, HL, HH), non-separable multi-dimensional wavelets (Quincunx sampling), Wavelet Packets, best basis selection (Shannon entropy). |
| `PE-EC703C_Module6_Notes.pdf` | Wavelet Transform and Applications: Transform coding, DWT vs. DCT (JPEG 2000 vs. JPEG), subband audio/video coding, wavelet denoising (hard/soft thresholding, universal VisuShrink, adaptive SureShrink), speckle homomorphic filtering, edge detection, object isolation, image fusion, discrete wavelet multi-tone modulation (DWMT vs. OFDM). |
| `PE-EC703C_Module7_Notes.pdf` | Beyond Wavelets (Ridgelets and Curvelets): Limitations of 2D wavelets (isotropic support, curve representations), Ridgelet transform (Radon projection), Curvelet transform (parabolic scaling, USFFT, wrapping), properties and applications of Ridgelets and Curvelets. |
| `PE-EC703C_Wavelet_Transforms.pdf` | **Combined** — all 7 modules, 42 pages |

...
\begin{tikzpicture}[scale=1.1, thick, font=\small]
  % Draw three axes radiating from center
  \draw[line, ->] (0, 0) -- (-3.2, 1.85) node[left, color=myred] {\textbf{Behavioral Domain}};
  \draw[line, ->] (0, 0) -- (3.2, 1.85) node[right, color=myteal] {\textbf{Structural Domain}};
  \draw[line, ->] (0, 0) -- (0, -3.2) node[below, color=mygreen] {\textbf{Physical Domain}};

  % Abstraction levels markers (Behavioral)
  \filldraw[myred] (-2.4, 1.385) circle (2pt) node[above left, font=\scriptsize] {Algorithm};
  \filldraw[myred] (-1.6, 0.924) circle (2pt) node[above left, font=\scriptsize] {RTL};
  \filldraw[myred] (-0.8, 0.462) circle (2pt) node[above left, font=\scriptsize] {Logic / Gate};
  \filldraw[myred] (-0.4, 0.231) circle (2pt);

  % Abstraction levels markers (Structural)
  \filldraw[myteal] (2.4, 1.385) circle (2pt) node[above right, font=\scriptsize] {Processor / Memory};
  \filldraw[myteal] (1.6, 0.924) circle (2pt) node[above right, font=\scriptsize] {ALU / Register};
  \filldraw[myteal] (0.8, 0.462) circle (2pt) node[above right, font=\scriptsize] {Gate / Flip-flop};
  \filldraw[myteal] (0.4, 0.231) circle (2pt);

  % Abstraction levels markers (Physical)
  \filldraw[mygreen] (0, -2.4) circle (2pt) node[right, font=\scriptsize] {Chip / Floorplan};
  \filldraw[mygreen] (0, -1.6) circle (2pt) node[right, font=\scriptsize] {Module / Cell Layout};
  \filldraw[mygreen] (0, -0.8) circle (2pt) node[right, font=\scriptsize] {Gate Layout};

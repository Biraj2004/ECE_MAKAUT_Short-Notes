...
\begin{itemize}[leftmargin=*, itemsep=3pt]
  ...
  \begin{itemize}[leftmargin=*, itemsep=2.5pt, topsep=0pt]
    ...
    \item If the RAG contains a cycle $\to$
      ...
      \item If multiple instances per resource type: Deadlock *may* exist (cycle is necessary but not sufficient).
    \end{itemize}
  \end{itemize}
...
\begin{itemize}[leftmargin=*, itemsep=3pt]
  \item \textbf{Prevent Mutual Exclusion:} Make resources shareable (e.g., read-only files). *Drawback:* Some resources (printers, mutexes) are inherently non-shareable.
  \item \textbf{Prevent Hold and Wait:} Require processes to request all resources at once before execution, or release current resources before requesting new ones. *Drawback:* Low resource utilization; starvation.
  \item \textbf{Prevent No Preemption:} If a process holding resources requests another that cannot be immediately allocated, all its current resources are preempted (released). *Drawback:* Overhead of saving/restoring process states.
  \item \textbf{Prevent Circular Wait:} Impose a global ordering on all resource types (e.g., $F(R_j) = i$) and require processes to request resources only in strictly increasing order of enumeration. *Advantage:* Most practical prevention strategy.
\end{itemize}
...
\begin{enumerate}[topsep=0pt, label=\textbf{Q\arabic*.}, itemsep=5pt]
  ...
  \item \Q{Consider a system with 5 processes (P0 to P4) and 3 resource types (A, B, C):}\\
    ...
    *Step 2: Initialize $\text{Work} = \begin{pmatrix} 3 \& 3 \& 2 \end{pmatrix}$*\par
    - Check P1: $\text{Need}_1 \begin{pmatrix} 1 \& 2 \& 2 \end{pmatrix} \le \text{Work} \begin{pmatrix} 3 \& 3 \& 2 \end{pmatrix}$? Yes. $\text{Work} \to \text{Work} + \text{Alloc}_1 = \begin{pmatrix} 5 \& 3 \& 2 \end{pmatrix}$.\par
    - Check P3: $\text{Need}_3 \begin{pmatrix} 0 \& 1 \& 1 \end{pmatrix} \le \text{Work} \begin{pmatrix} 5 \& 3 \& 2 \end{pmatrix}$? Yes. $\text{Work} \to \text{Work} + \text{Alloc}_3 = \begin{pmatrix} 7 \& 4 \& 3 \end{pmatrix}$.\par
    - Check P0: $\text{Need}_0 \begin{pmatrix} 7 \& 4 \& 3 \end{pmatrix} \le \text{Work} \begin{pmatrix} 7 \& 4 \& 3 \end{pmatrix}$? Yes. $\text{Work} \to \text{Work} + \text{Alloc}_0 = \begin{pmatrix} 7 \& 5 \& 3 \end{pmatrix}$.\par
    - Check P2: $\text{Need}_2 \begin{pmatrix} 6 \& 0 \& 0 \end{pmatrix} \le \text{Work} \begin{pmatrix} 7 \& 5 \& 3 \end{pmatrix}$? Yes. $\text{Work} \to \text{Work} + \text{Alloc}_2 = \begin{pmatrix} 10 \& 5 \& 5 \end{pmatrix}$.\par
    - Check P4: $\text{Need}_4 \begin{pmatrix} 4 \& 3 \& 1 \end{pmatrix} \le \text{Work} \begin{pmatrix} 10 \& 5 \& 5 \end{pmatrix}$? Yes. $\text{Work} \to \text{Work} + \text{Alloc}_4 = \begin{pmatrix} 10 \& 5 \& 7 \end{pmatrix}$.\par
    *Result:* All processes finished. System is in \textbf{safe state} with safe sequence: $\langle \mathbf{P1, P3, P0, P2, P4} \rangle$.
...
\begin{itemize}[leftmargin=*, itemsep=3pt]
  \item \textbf{Single-Level Directory:} All files are in a single directory. *Problems:* Naming collisions, organization issues.
...
1. \textbf{Bit Vector (Bit Map):} An array of bits where each bit corresponds to a disk block (0 = allocated, 1 = free). *Advantages:* Fast, easy to find consecutive blocks. *Disadvantages:* Must be kept in RAM, consumes space for large disks.
2. \textbf{Linked List:} All free blocks are linked together. Directory keeps a pointer to the first free block. *Advantages:* Minimal space overhead. *Disadvantages:* Slow to traverse; hard to find contiguous free blocks.
...
\begin{enumerate}[topsep=0pt, label=\textbf{Q\arabic*.}, itemsep=5pt]
  ...
  \item \Q{Describe free space management using a Bit Vector. Calculate the size of a bit map for a 200 GB disk with 4 KB blocks.}
    ...
    *Total Blocks* = $200\,\text{GB} / 4\,\text{KB} = (200 \times 10^9) / (4 \times 10^3) = 50 \times 10^6$ blocks.\par
    *Map Size* = $50 \times 10^6$ bits $\approx 6.25\,\text{MB}$ of storage.
...
\begin{enumerate}[topsep=0pt, label=\textbf{Q\arabic*.}, itemsep=5pt]
  ...
  \item \Q{Explain the UNIX Inode pointer layout and calculate max file size for a 2 KB block size with 4-byte pointers.}
    ...
    *Pointers per block* = $2\,\text{KB} / 4\,\text{B} = 512$.\par
    *Direct size* = $12 \times 2\,\text{KB} = 24\,\text{KB}$.\par
    *Single indirect size* = $512 \times 2\,\text{KB} = 1024\,\text{KB} = 1\,\text{MB}$.\par
    *Double indirect size* = $512^2 \times 2\,\text{KB} = 512\,\text{MB}$.\par
    *Triple indirect size* = $512^3 \times 2\,\text{KB} = 256\,\text{GB}$.\par
    *Max File Size* = $24\,\text{KB} + 1\,\text{MB} + 512\,\text{MB} + 256\,\text{GB} \approx 256.5\,\text{GB}$.

...
\hline
\rowcolor{hdrteal}
\textbf{Band} & \textbf{Uplink Range} & \textbf{Downlink Range} & \textbf{Key Advantages / Applications} \\
\hline
\textbf{L Band} & $1.61 - 1.66$ GHz & $1.52 - 1.56$ GHz & Low attenuation, small antennas; GPS, mobile satellite terminals (Iridium). \\
\hline
\rowcolor{rowalt}
\textbf{S Band} & $2.1 - 2.3$ GHz & $1.9 - 2.1$ GHz & Minimal rain fade; Mobile satellite service, DTH audio broadcasting. \\
\hline
\textbf{C Band} & $5.92 - 6.42$ GHz & $3.7 - 4.2$ GHz & Highly mature technology, low rain fade; Telephony, TV networks, VSAT systems. \\
\hline
\rowcolor{rowalt}
\textbf{X Band} & $7.9 - 8.4$ GHz & $7.25 - 7.75$ GHz & Highly secure, military exclusive; Reconnaissance, military communications. \\
\hline
\textbf{Ku Band} & $14.0 - 14.5$ GHz & $11.7 - 12.2$ GHz & High gain, smaller dish size; DTH TV, VSAT networks, high-speed data. \\
\hline
\rowcolor{rowalt}
\textbf{Ka Band} & $27.5 - 31.0$ GHz & $17.7 - 21.2$ GHz & Massive bandwidth; Satellite broadband (Starlink, Viasat), high-throughput systems. \\
\hline
\end{tabularx}
...
\hline
\rowcolor{hdrgray}
\textbf{Parameter} & \textbf{\color{myteal}C Band} & \textbf{\color{myred}Ku Band} & \textbf{\color{myamber}Ka Band} \\
\hline
\textbf{Frequency (U/D)} & $6 / 4$ GHz & $14 / 12$ GHz & $30 / 20$ GHz \\
\hline
\textbf{Rain Fade} & Negligible / Low & Moderate & Very High \\
\hline
\textbf{Dish Antenna Size} & Large ($2.4 - 3.7$ m) & Small ($0.6 - 1.2$ m) & Very Small ($0.3 - 0.75$ m) \\
\hline
\textbf{Applications} & Backhaul telephony, TV distribution, maritime links. & DTH satellite TV, home VSAT terminals. & Satellite broadband, high-throughput commercial services. \\
\hline
\end{tabularx}
\end{tcolorbox}
...
\renewcommand{\arraystretch}{1.3}
\begin{tabularx}{\linewidth}{B{5.0cm} X}
LO Frequency Shift & $f_{LO} = f_{uplink} - f_{downlink}$ \\[4pt]
...
\hline
\rowcolor{hdrgray}
\textbf{Feature} & \textbf{\color{myteal}Bent-Pipe Transponder} & \textbf{\color{mypurple}Regenerative Transponder} \\
\hline
\textbf{Basic Operation} & RF-to-RF translation & Demodulation $\to$ Baseband Processing $\to$ Remodulation \\
\hline
\textbf{Noise Propagation} & Uplink noise is amplified and sent down. & Uplink noise is removed at baseband. \\
\hline
\textbf{Complexity \& Weight} & Low complexity, lightweight, highly reliable. & High complexity, heavy, requires onboard processors. \\
\hline
\textbf{Flexibility} & Protocol transparent (handles any modulation format). & Protocol-specific (locked to a particular modulation/coding standard). \\
\hline
\textbf{Primary Use Case} & Standard satellite TV distribution, VSAT links. & Modern satellite broadband, military secure links. \\
\hline
\end{tabularx}
\end{tcolorbox}
...
\hline
Transmitter Power ($P_t$) & $10 \text{ dBW}$ & $10 \text{ dBW}$ \\
\hline
Transmit Antenna Gain ($G_t$) & $45 \text{ dBi}$ & $45 \text{ dBi}$ \\
\hline
\textbf{EIRP} & $\mathbf{55 \text{ dBW}}$ & $\mathbf{55 \text{ dBW}}$ \\
\hline
Free Space Loss ($L_{FSL}$) & $-200 \text{ dB}$ & $-200 \text{ dB}$ \\
\hline
Atmospheric/Rain Loss ($L_a$) & $-0.5 \text{ dB}$ & $-3.5 \text{ dB}$ (includes rain) \\
\hline
Receiver Antenna Gain ($G_r$) & $35 \text{ dBi}$ & $35 \text{ dBi}$ \\
\hline
System Noise Temp ($T_s$) & $150 \text{ K}$ ($21.8 \text{ dB-K}$) & $250 \text{ K}$ ($24.0 \text{ dB-K}$) (increases due to rain) \\
\hline
Boltzmann Constant ($k$) & $-228.6 \text{ dBW/Hz-K}$ & $-228.6 \text{ dBW/Hz-K}$ \\
\hline
\textbf{C/N$_0$ Ratio} & $\mathbf{96.3 \text{ dB-Hz}}$ & $\mathbf{91.1 \text{ dB-Hz}}$ \\
\hline
\end{tabularx}
\end{tcolorbox}
...
\hline
\rowcolor{hdrgray}
\textbf{Parameter} & \textbf{\color{myteal}FDMA} & \textbf{\color{myred}TDMA} & \textbf{\color{myamber}CDMA} \\
\hline
\textbf{Resource Allocation} & Dedicated frequency slots, continuous transmission. & Dedicated time slots, full bandwidth bursts. & Shared time and frequency, separated by codes. \\
\hline
\textbf{IMD Vulnerability} & High (requires back-off). & Negligible (only one carrier in transponder at a time). & Low (mitigated by code orthogonality). \\
\hline
\textbf{Complexity} & Low; simple frequency filters at receiver. & High; requires precise time synchronization. & High; requires complex code generators and correlators. \\
\hline
\textbf{Flexibility} & Rigid; channel bandwidth is fixed. & Flexible; slots can be dynamically reallocated. & High; soft capacity limit (adding users degrades noise floor). \\
\hline
\end{tabularx}
\end{tcolorbox}

...
\hline
\textbf{Flat Fading} & $B_s \ll B_c$ \;/\; $T_s \gg \sigma_\tau$ & Amplitude fluctuates, pulse shape preserved. No ISI. \\
\hline
\textbf{Freq. Selective} & $B_s > B_c$ \;/\; $T_s < \sigma_\tau$ & Signal distorted, severe ISI. Requires equalizer. \\
\hline
\rowcolor{rowalt}
\textbf{Fast Fading} & $T_s > T_c$ \;/\; $B_s < B_D$ & Channel changes within symbol. Distorts envelope. \\
\hline
\textbf{Slow Fading} & $T_s \ll T_c$ \;/\; $B_s \gg B_D$ & Channel constant over many symbols. Normal detection. \\
\hline
\end{tabularx}
\tcbset{colbacktitle=myteal, coltitle=white}
\end{tcolorbox}
...
\hline
\rowcolor{hdrgray}
\textbf{Parameter} & \textbf{\color{myteal}Monopole Antenna} & \textbf{\color{myred}Planar Inverted-F (PIFA)} \\
\hline
\textbf{Structure} & Simple vertical rod over a ground plane. & Flat rectangular patch parallel to a ground plane with a shorting pin. \\
\hline
\textbf{Resonant Length} & $\lambda/4$ (vertical height). & $\approx \lambda/4$ (sum of length and width, $L+W$). \\
\hline
\rowcolor{rowalt}
\textbf{Profile} & High profile (protruding structure). & Low profile, easily embedded inside devices. \\
\hline
\textbf{SAR (Radiation towards head)} & High (radiates symmetrically around the rod). & Low (ground plane acts as an electromagnetic shield). \\
\hline
\end{tabularx}
\tcbset{colbacktitle=myteal, coltitle=white}
\end{tcolorbox}
...
\hline
\textbf{Primary Goal} & Maximizes data rate (throughput). & Maximizes link reliability (reduces BER). \\
\hline
\textbf{Transmission} & Transmits different data streams from each antenna simultaneously. & Transmits the same or coded data streams from multiple antennas. \\
\hline
\rowcolor{rowalt}
\textbf{Capacity Scaling} & Scales linearly with $\min(N_T, N_R)$. & No rate increase, but stabilizes signal SNR. \\
\hline
\textbf{Key Schemes} & V-BLAST (Bell Labs Layered Space-Time). & Alamouti code, Space-Time Block Coding (STBC). \\
\hline
\rowcolor{rowalt}
\textbf{Channel Condition} & Requires high SNR and rich scattering. & Effective in poor SNR and deep fades. \\
\hline
\end{tabularx}
...
\hline
\rowcolor{hdrgray}
\textbf{Feature} & \textbf{GSM (2G)} & \textbf{GPRS (2.5G)} & \textbf{EDGE (2.75G)} & \textbf{WCDMA (3G)} \\
\hline

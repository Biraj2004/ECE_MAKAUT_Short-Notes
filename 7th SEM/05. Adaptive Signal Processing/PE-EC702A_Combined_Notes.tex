...
\textbf{Application Class} & \textbf{Input Signal $x(n)$} & \textbf{Desired Signal $d(n)$} & \textbf{Objective} \\[4pt]
\hline
\textbf{System ID} & Reference excitation & Unknown system output & Find model $\mathbf{w}(n) \approx \text{system}$ \\[4pt]
\hline
\textbf{Inverse Model} & Distorted signal & Original source signal & Recover original source \\
\hline
\textbf{Noise Cancellation} & Noise reference source & Corrupted signal & Extract clean signal $e(n)$ \\[4pt]
\hline
\textbf{Prediction} & Delayed signal $s(n-\Delta)$ & Current signal $s(n)$ & Predict future signal values \\[4pt]
\hline
\end{tabularx}
}
...
\begin{tabularx}{\linewidth}{B{4.5cm} X}Filter Output (FIR) & $y(n) = \mathbf{w}^T(n)\mathbf{x}(n) = \sum_{k=0}^{M-1} w_k(n) x(n-k)$ \\[6pt]
...
\hline
\textbf{Filter Input $x(n)$} & Excitation signal & Reference noise source & Distorted channel output \\[4pt]
\hline
\textbf{Desired Input $d(n)$} & Output of unknown system & Corrupted signal & Known training signal \\[4pt]
\hline
\textbf{Desired Output} & Model of system parameters & Cleaned signal $e(n)$ & Reconstructed original data \\[4pt]
\hline
\textbf{Key Application} & Echo cancellation in telephony & Active noise control, EEG & Wireless communications \\[4pt]
\hline
\end{tabularx}
}
\end{tcolorbox}
...
\begin{tcolorbox}[formulabox, title={\textbf{Key Formulas at a Glance}}]
\renewcommand{\arraystretch}{1.3}
\begin{tabularx}{\linewidth}{B{4.5cm} X}Wiener-Hopf Equation & $\mathbf{R} \mathbf{w}_o = \mathbf{p} \implies \mathbf{w}_o = \mathbf{R}^{-1} \mathbf{p}$ \\[6pt]
...
\begin{tcolorbox}[formulabox, title={\textbf{Key Formulas at a Glance}}]
\renewcommand{\arraystretch}{1.3}
\begin{tabularx}{\linewidth}{B{4.5cm} X}Forward Error & $f_m(n) = f_{m-1}(n) - \kappa_m^* g_{m-1}(n-1)$ \\[6pt]
...
\begin{tcolorbox}[formulabox, title={\textbf{Key Formulas at a Glance}}]
\renewcommand{\arraystretch}{1.3}
\begin{tabularx}{\linewidth}{B{4.5cm} X}RLS Cost Function & $\mathcal{E}(n) = \sum_{i=1}^n \lambda^{n-i} |e(i, n)|^2$ \\[6pt]
...
\hline
\textbf{Convergence Rate} & Slow (dependent on input eigenvalue spread) & Fast (typically within $2M$ iterations, independent of spread) \\[4pt]
\hline
\textbf{Complexity} & $O(M)$ multiplications per sample & $O(M^2)$ multiplications per sample \\[4pt]
\hline
\textbf{Stability} & Stable if $0 < \mu < 2/\text{Tr}(\mathbf{R})$ & Can suffer from numerical instability in finite precision \\[4pt]
\hline

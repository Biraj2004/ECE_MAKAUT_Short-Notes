...
\centering
\begin{tikzpicture}[
  block/.style={rectangle, draw=myteal, fill=hdrteal, text width=2.0cm, align=center, minimum height=0.75cm, font=\scriptsize\bfseries, rounded corners=3pt, line width=0.7pt},
  arrow/.style={-Stealth, thick, color=mydark}
]
  % Row 1: Zero-Level
  \node[block] (m1) at (0, 2.2) {Manufacturer};
  \node[block] (c1) at (10.5, 2.2) {Consumer};
  \draw[arrow] (m1) -- node[above, font=\tiny\bfseries\color{myred}] {Direct (Zero-Level Channel)} (c1);
  
  % Row 2: 1-Level
  \node[block] (m2) at (0, 1.1) {Manufacturer};
  \node[block] (r2) at (5.25, 1.1) {Retailer};
  \node[block] (c2) at (10.5, 1.1) {Consumer};
  \draw[arrow] (m2) -- (r2);
  \draw[arrow] (r2) -- node[above, font=\tiny\bfseries\color{myteal}] {Indirect (1-Level Channel)} (c2);
  
  % Row 3: 2-Level
  \node[block] (m3) at (0, 0) {Manufacturer};
  \node[block] (w3) at (3.5, 0) {Wholesaler};
  \node[block] (r3) at (7.0, 0) {Retailer};
  \node[block] (c3) at (10.5, 0) {Consumer};
  \draw[arrow] (m3) -- (w3);
  \draw[arrow] (w3) -- (r3);
  \draw[arrow] (r3) -- node[above, font=\tiny\bfseries\color{myamber}] {Indirect (2-Level Channel)} (c3);
\end{tikzpicture}
\end{tcolorbox}
...
\begin{tikzpicture}[scale=0.85]
  % Define BEP coordinates
  \coordinate (BEP) at (5.33,3.67);
  
  % Fill Loss and Profit Zones first (in background)
  \fill[myred!12] (0,2) -- (BEP) -- (0,0) -- cycle;
  \fill[mygreen!12] (BEP) -- (8,5.5) -- (8,4.5) -- cycle;
...
\subsection{Goods and Services Tax (GST)}
Implemented on July 1, 2017, the Goods and Services Tax (GST) is a comprehensive, multi-stage, destination-based indirect tax that subsumed almost all traditional central and state indirect taxes (like Excise Duty, VAT, Service Tax, and Entry Tax) under the slogan ``One Nation, One Tax''.

\begin{tcolorbox}[defbox, title={Core Components of GST}]
GST in India is structured as a dual model, where both Center and States levy tax simultaneously:
\begin{itemize}[leftmargin=*, itemsep=2pt, topsep=0pt]
  \item \textbf{CGST (Central GST):} Collected by the Central Government on intra-state transactions.
  \item \textbf{SGST (State GST) / UTGST:} Collected by State Governments / Union Territories on intra-state transactions.
  \item \textbf{IGST (Integrated GST):} Collected by the Central Government on inter-state transactions and imports, which is then shared between the Center and the destination State.
\end{itemize}
\end{tcolorbox}

\subsubsection{Key Features and Slabs of GST}
\begin{itemize}[leftmargin=*, itemsep=3.5pt]
  \item \textbf{Destination-Based Tax:} Unlike traditional sales tax (which was origin-based), GST is collected by the state where goods or services are finally consumed.
  \item \textbf{Input Tax Credit (ITC) Extension:} GST creates a seamless credit chain across the entire supply chain, completely removing the cascading ``tax on tax'' effect.
  \item \textbf{Tax Slabs:} Goods and services are taxed under four primary rate slabs: 5\%, 12\%, 18\%, and 28\%. Basic essentials (like food grains) are kept at 0\% (exempt), while luxury/demerit goods attract 28\% plus additional cess.
  \item \textbf{Exclusions:} Petroleum products, alcohol for human consumption, and electricity are currently kept outside the purview of GST and continue to be taxed under traditional excise and VAT regimes.
\end{itemize}

\subsection{Comparison: Pre-GST vs. Post-GST Indirect Tax Regimes}
\par\noindent
{\renewcommand{\arraystretch}{1.5}%
\begin{tabularx}{\linewidth}{|B{3.0cm}|Y|Y|}\hline
\multicolumn{1}{|>{\centering\arraybackslash}m{3.0cm}|}{\textbf{Parameter}} &
\multicolumn{1}{>{\centering\arraybackslash}Y|}{\textbf{Pre-GST Regime}} &
\multicolumn{1}{>{\centering\arraybackslash}Y|}{\textbf{Post-GST Regime}} \\
\hline
\textbf{Tax Structure} & Fragmented; multiple taxes (Excise, VAT, CST, Entry Tax, Service Tax). & Unified; single indirect tax (GST) subsuming multiple levies. \\
\hline
\textbf{Taxation Point} & Origin-based at different stages (production, sales, service entry). & Destination-based; charged at the point of consumption. \\
\hline
\textbf{Cascading Effect} & High; tax on tax was common due to lack of cross-input credit. & Eliminated; seamless flow of Input Tax Credit (ITC) from manufacturer to retailer. \\
\hline
\textbf{Compliance} & Complex; separate filings and registers for each tax authority. & Simplified; single online portal (GSTN) for registration and filing. \\
\hline
\end{tabularx}
}
...
\begin{tikzpicture}[scale=0.95]
  % Triangle Coordinates (Base width=5, height=5, centered around x=2.5)
  \coordinate (A) at (0,0);
  \coordinate (B) at (5,0);
  \coordinate (C) at (2.5,5);
  
  % Level 5 (Physiological)
  \fill[hdrteal] (0,0) -- (5,0) -- (4.5,1) -- (0.5,1) -- cycle;
  \draw[thick, color=mydark] (0,0) -- (5,0) -- (4.5,1) -- (0.5,1) -- cycle;
  \node[font=\footnotesize\bfseries\color{myteal}] at (2.5,0.45) {Level 5};
  \draw[dashed, thick, color=mydark!60] (4.75,0.5) -- (5.7,0.5);
  \node[anchor=west, align=left, font=\scriptsize] at (5.8,0.5) {\textbf{Physiological Needs}: Food, water, shelter, sleep};
  
  % Level 4 (Safety)
  \fill[hdramber] (0.5,1) -- (4.5,1) -- (4,2) -- (1,2) -- cycle;
  \draw[thick, color=mydark] (0.5,1) -- (4.5,1) -- (4,2) -- (1,2) -- cycle;
  \node[font=\footnotesize\bfseries\color{myamber}] at (2.5,1.45) {Level 4};
  \draw[dashed, thick, color=mydark!60] (4.25,1.5) -- (5.7,1.5);
  \node[anchor=west, align=left, font=\scriptsize] at (5.8,1.5) {\textbf{Safety \& Security}: Job stability, health protection};
  
  % Level 3 (Social)
  \fill[hdrpurple] (1,2) -- (4,2) -- (3.5,3) -- (1.5,3) -- cycle;
  \draw[thick, color=mydark] (1,2) -- (4,2) -- (3.5,3) -- (1.5,3) -- cycle;
  \node[font=\footnotesize\bfseries\color{mypurple}] at (2.5,2.45) {Level 3};
  \draw[dashed, thick, color=mydark!60] (3.75,2.5) -- (5.7,2.5);
  \node[anchor=west, align=left, font=\scriptsize] at (5.8,2.5) {\textbf{Social \& Belonging}: Friendship, love, team inclusion};
  
  % Level 2 (Esteem)
  \fill[hdrgreen] (1.5,3) -- (3.5,3) -- (3,4) -- (2,4) -- cycle;
  \draw[thick, color=mydark] (1.5,3) -- (3.5,3) -- (3,4) -- (2,4) -- cycle;
  \node[font=\footnotesize\bfseries\color{mygreen}] at (2.5,3.45) {Level 2};
  \draw[dashed, thick, color=mydark!60] (3.25,3.5) -- (5.7,3.5);
  \node[anchor=west, align=left, font=\scriptsize] at (5.8,3.5) {\textbf{Esteem Needs}: Status, recognition, respect, pride};
  
  % Level 1 (Self-Actualization)
  \fill[hdrred] (2,4) -- (3,4) -- (2.5,5) -- cycle;
  \draw[thick, color=mydark] (2,4) -- (3,4) -- (2.5,5) -- cycle;
  \node[font=\footnotesize\bfseries\color{myred}] at (2.5,4.45) {Level 1};
  \draw[dashed, thick, color=mydark!60] (2.75,4.5) -- (5.7,4.5);
  \node[anchor=west, align=left, font=\scriptsize] at (5.8,4.5) {\textbf{Self-Actualization}: Personal growth, self-fulfillment};
\end{tikzpicture}
\end{tcolorbox}

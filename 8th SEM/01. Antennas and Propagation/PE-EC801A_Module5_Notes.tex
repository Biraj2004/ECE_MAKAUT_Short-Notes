...
\textbf{Resonant Mode:} The superscript $z$ indicates that the $z$-component of the magnetic vector potential $A_z$ is used. The subscripts $(1,1,0)$ denote: 1 variation in $\rho$ (radial), 1 variation in $\phi$ (azimuthal), and 0 variation in $z$. This mode produces broadside radiation (maximum perpendicular to the patch plane) and is the dominant mode for most designs. The constant $1.8412$ is the first zero of the derivative of the Bessel function $J_1'(x) = 0$.

\textbf{Effect of Fringing Fields:} Just as in the rectangular patch, the fringing fields at the circular patch edge make it electrically larger than its physical radius $a$. The effective radius $a_e > a$ accounts for this extension.

\textbf{Design Procedure:}
\begin{enumerate}[label=\textbf{\arabic*.}, leftmargin=*, itemsep=3pt]
  \item Choose substrate parameters: $\epsilon_r$ and $h$ based on availability and bandwidth requirements.
  \item Calculate the intermediate parameter $F$ from the desired resonant frequency $f_r$.
  \item Solve for physical radius $a$ (which already includes the first fringing correction).
  \item Calculate the effective radius $a_e$ for verification.
  \item Verify the resonant frequency using $(f_r)_{110}$.
\end{enumerate}

\textbf{Typical values:} For a circular patch at $2.4\,\text{GHz}$ on FR-4 substrate ($\epsilon_r = 4.4$, $h = 1.6\,\text{mm}$):
\begin{itemize}[leftmargin=*, itemsep=2pt]
  \item $F \approx 17.5\,\text{mm}$, $a \approx 16.8\,\text{mm}$, $a_e \approx 17.2\,\text{mm}$
  \item Bandwidth $\approx 1\%$–$3\%$, similar to rectangular patch
\end{itemize}

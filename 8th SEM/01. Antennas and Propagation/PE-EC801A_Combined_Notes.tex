...
\begin{tikzpicture}[scale=0.95]
  ...
  \begin{scope}[xshift=-3.8cm]
    ...
    \node[below, font=\tiny\bfseries] at (0,-1.3) {Ground Plane (PEC)};
    % Substrate
    \draw[thick, fill=hdrteal, fill opacity=0.35] (-2.0,-1.0) rectangle (2.0,0.0);
    \node[font=\tiny\bfseries\color{myteal}] at (0,-0.5) {Substrate ($\epsilon_r$, $h$)};
    % Patch
    \draw[very thick, color=myred, fill=hdrred] (-1.2,0.0) rectangle (1.2,0.18);
    \node[above, font=\scriptsize\bfseries\color{myred}] at (0,0.18) {Metallic Patch};
    ...
    \node[font=\tiny\bfseries\color{myamber}] at (-1.95, -0.35) {Fringing};
    \node[font=\tiny\bfseries\color{myamber}] at (-1.95, -0.55) {Fields};
    % Height annotation
    \draw[Stealth-Stealth, thick, color=mydark] (2.1,-1.0) -- (2.1, 0.0) node[midway, right, font=\tiny] {$h$};
    \node[font=\small\bfseries\color{mydark}] at (0,-1.65) {(a) Cross-Section View};
  \end{scope}
...
\centering
\begin{tikzpicture}[scale=0.9]
  % Microstrip Line Feed (top view)
  \begin{scope}[xshift=-4cm]
    \draw[thick, fill=hdrgray] (-2,-2) rectangle (2,2);
    \draw[very thick, fill=myred] (-1.2,-1.2) rectangle (1.2,1.2);
    % Inset Feed
    \fill[white] (-0.25,-1.2) rectangle (0.25,-0.6);
    \draw[thick] (-0.25,-1.2) -- (-0.25,-0.6) -- (0.25,-0.6) -- (0.25,-1.2);
    \draw[very thick, fill=myamber] (-0.1,-2) rectangle (0.1,-0.6);
    \node[above, font=\tiny] at (0,-0.6) {Inset feed};
    \node[font=\small\bfseries\color{mydark}] at (0,-2.3) {(a) Inset Microstrip Line Feed};
  \end{scope}

  % Coaxial Probe Feed (side cross section)
  \begin{scope}[xshift=3.5cm]
    % Ground plane
    \draw[line width=3pt, color=mgframe] (-2.2,-1.0) -- (2.2,-1.0);
    \node[below, font=\tiny] at (0,-1.0) {Ground Plane};
    % Substrate
    \draw[thick, fill=hdrteal, fill opacity=0.3] (-2.2,-1.0) rectangle (2.2,0.2);
    \node[left, font=\tiny\color{myteal}] at (-1.8,-0.4) {Substrate};
    % Patch
    \draw[very thick, color=myred] (-1.5,0.2) -- (1.5,0.2);
    \node[above, font=\tiny\color{myred}] at (0,0.2) {Patch};
    
    % Coaxial pin
    \draw[line width=1.5pt, color=myamber] (0.5,-1.8) -- (0.5,0.2);
    \draw[line width=3.5pt, color=mydark, dashed] (0.3,-1.8) -- (0.3,-1.0);
    \draw[line width=3.5pt, color=mydark, dashed] (0.7,-1.8) -- (0.7,-1.0);
    \node[right, font=\tiny\color{myamber}] at (0.5,-1.4) {Inner Pin};
    \node[font=\small\bfseries\color{mydark}] at (0,-2.3) {(b) Coaxial Probe Feed};
  \end{scope}
\end{tikzpicture}
\end{tcolorbox}
...
\]
where $z_i$ are the roots (zeros) of the polynomial. Since $z$ lies on the unit circle ($|z| = 1$), the nulls in the radiation pattern correspond to the roots that lie on the unit circle. This allows designers to synthesize specific radiation pattern shapes by selecting root locations on the complex plane.

\subsection{Woodward-Lawson Method}
The Woodward-Lawson method synthesizes a desired source pattern by superimposing several independent, orthogonal beams (beams centered at different angles, each having a $\frac{\sin(N\psi/2)}{N\sin(\psi/2)}$ shape). The excitation coefficients of the array are found by sampling the desired pattern at the center coordinates of these orthogonal beams.

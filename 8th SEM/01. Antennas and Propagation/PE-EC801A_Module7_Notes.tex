...
\begin{tikzpicture}[node distance=0.4cm and 0.7cm,
  ...
  ablock/.style={circle, draw=mypurple, fill=hdrpurple,
    minimum size=0.7cm, font=\small, line width=0.7pt}]

  % ── Antenna elements ──────────────────────────────────────────────────────
  \node[ablock] (a1) at (0,  1.4) {Ant 1};
  \node[ablock] (a2) at (0,  0.0) {Ant 2};
  \node[font=\large, color=mgframe] at (0, -0.8) {$\vdots$};
  \node[ablock] (aN) at (0, -1.6) {Ant N};

  % ── RF Downconverters ─────────────────────────────────────────────────────
  \node[sblock, right=0.7cm of a1] (rf1) {RF Down\\Converter};
  \node[sblock, right=0.7cm of a2] (rf2) {RF Down\\Converter};
  \node[sblock, right=0.7cm of aN] (rfN) {RF Down\\Converter};

  % ── ADCs ─────────────────────────────────────────────────────────────────
  \node[sblock, right=0.6cm of rf1] (adc1) {ADC};
  \node[sblock, right=0.6cm of rf2] (adc2) {ADC};
  \node[sblock, right=0.6cm of rfN] (adcN) {ADC};

  % ── DSP Unit ──────────────────────────────────────────────────────────────
  \node[draw=myred, fill=hdrred, line width=0.9pt, rounded corners=5pt,
        minimum height=4.2cm, text width=2.3cm, align=center,
        right=0.8cm of adc2] (dsp)
       {\textbf{DSP Unit}\\[4pt]
        \scriptsize DOA\\[-2pt]Estimation\\[3pt]
        \scriptsize Weight\\[-2pt]Update\\[3pt]
        \scriptsize Beam-\\[-2pt]forming};
  ...
  \draw[arrow] (rf2) -- (adc2);
  \draw[arrow] (rfN) -- (adcN);
  \draw[arrow] (adc1.east) -- ++(0.4,0) |- (dsp.160);
  \draw[arrow] (adc2.east) -- (dsp.west);
  \draw[arrow] (adcN.east) -- ++(0.4,0) |- (dsp.200);

  % ── Dots between rf2/adcN rows ─────────────────────────────────────────────
  \node[font=\large, color=mgframe] at ([xshift=1.2cm]0,-0.8) {$\vdots$};
  \node[font=\large, color=mgframe] at ([xshift=3.3cm]0,-0.8) {$\vdots$};

  % ── Output ───────────────────────────────────────────────────────────────
  \draw[arrow, line width=1.2pt] (dsp.east) -- ++(1.5,0)

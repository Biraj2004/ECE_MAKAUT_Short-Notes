...
\begin{tcolorbox}[tikzbox, title={IoT 3-Tier Architecture}]
\centering
\begin{tikzpicture}[scale=0.95, transform shape]
  % Cloud Layer
  \node[block, draw=myred, fill=hdrred, text width=3.2cm] (cloud) at (0,3.6) {\textbf{Application Layer}\\Cloud Server / Database\\Data Analytics};
  
  % Gateway Layer
  \node[block, draw=myteal, fill=hdrteal, text width=3.2cm] (gate) at (0,1.8) {\textbf{Network Layer}\\Gateway / Router\\Protocol Translation};
  
  % Edge Layer
  \node[block, draw=myamber, fill=hdramber, text width=3.2cm] (edge) at (0,0) {\textbf{Edge Layer}\\Sensors \& Actuators\\Smart Things};

  % Connections
  \draw[arrow, <->, line width=1.1pt] (edge) -- (gate);
  \draw[arrow, <->, line width=1.1pt] (gate) -- (cloud);

  % Details (Right side)
  \node[right=1.2cm of cloud, font=\scriptsize, align=left, color=mydark] {• Cloud APIs \& Dashboards\\• Big Data Analytics};
  \node[right=1.2cm of gate, font=\scriptsize, align=left, color=mydark] {• Wi-Fi / Cellular / LoRa\\• Packet Routing};
  \node[right=1.2cm of edge, font=\scriptsize, align=left, color=mydark] {• Raw Data Collection\\• Local Control Actions};
\end{tikzpicture}
\end{tcolorbox}
...
\begin{tcolorbox}[tikzbox, title={MQTT Publish-Subscribe Model}]
\centering
\begin{tikzpicture}[scale=0.9, transform shape, node distance=1.8cm]
  \node[block, draw=myamber, fill=hdramber] (pub) {\textbf{Publisher}\\(DHT11 Node)};
  \node[block, right=2.0cm of pub, draw=myred, fill=hdrred] (broker) {\textbf{MQTT Broker}\\(Central Server)};
  \node[block, right=2.0cm of broker, draw=myteal, fill=hdrteal] (sub) {\textbf{Subscriber}\\(Mobile App)};

  \draw[arrow] (pub) -- node[above, font=\scriptsize] {Publish: ``temp/25C''} (broker);
  \draw[arrow] (sub) -- node[above, font=\scriptsize] {Subscribe ``temp''} (broker);
  \draw[arrow, bend right=25] (broker) to node[below, font=\scriptsize] {Forward: ``temp/25C''} (sub);
\end{tikzpicture}
\end{tcolorbox}
...
\centering
\begin{tikzpicture}[scale=0.9, transform shape, node distance=2.2cm]
  \node[block, draw=myteal, fill=hdrteal] (client) {\textbf{CoAP Client}\\(Controller)};
  \node[block, right=3.5cm of client, draw=myamber, fill=hdramber] (server) {\textbf{CoAP Server}\\(Smart Valve)};

  \draw[arrow] (client.north east) -- node[above, font=\scriptsize] {GET /valve/status} (server.north west);
  \draw[arrow] (server.south west) -- node[below, font=\scriptsize] {2.05 Content (value=Open)} (client.south east);
\end{tikzpicture}
\end{tcolorbox}

...
\begin{itemize}[leftmargin=*, itemsep=2pt, topsep=0pt]
  ...
  \item \textbf{Frequency Warping:} Nonlinear frequency mapping introduced by the bilinear transformation.
\end{itemize}
\end{tcolorbox}
...
\begin{itemize}[leftmargin=*, itemsep=2pt, topsep=0pt]
  ...
  \item \textbf{Bottom-Plate Sampling:} A technique where the bottom plate of a capacitor is disconnected first to eliminate input-dependent charge injection.
\end{itemize}
\end{tcolorbox}
...
\begin{tikzpicture}[scale=0.9, transform shape]
  % Resistor ladder
  \draw[thick] (0, 3) node[above] {$V_{REF}$} to[R, l=$R$] (0, 2) to[R, l=$R$] (0, 1) to[R, l=$R$] (0, 0) to[R, l=$R$] (0, -1) to[R, l=$R$] (0, -2) to[R, l=$R$] (0, -3) to[R, l=$R$] (0, -4) -- (0, -4.5) node[ground] {};

  % Comparators
  \foreach \y/\i in {2.5/3, 1.5/2, 0.5/1, -0.5/0, -1.5/-1, -2.5/-2, -3.5/-3} {
    \draw[thick] (2.2, \y) -- (2.2, \y+0.4) -- (3.0, \y+0.2) -- cycle;
    \node[font=\tiny] at (2.4, \y+0.3) {$+$};
    \node[font=\tiny] at (2.4, \y+0.1) {$-$};
    \node[right, font=\tiny] at (3.0, \y+0.2) {$D_{\i}$};
  ...
  \draw[thick] (-2, 2.8) node[left] {$V_{in}$} -- (1.5, 2.8) -- (1.5, -3.2);
  \foreach \y in {2.5, 1.5, 0.5, -0.5, -1.5, -2.5, -3.5} {
    \draw (1.5, \y+0.3) -- (2.2, \y+0.3); % connects to positive
  }

  % Encoder Block
  \draw[thick, fill=hdrteal, draw=myteal] (4.2, -3.8) rectangle (5.8, 3.2);
  \node[rotate=-90, font=\small\bfseries, align=center] at (5.0, -0.3) {Priority\\Encoder};
  
  % Connections from comparators to encoder
  \foreach \y/\i in {2.5/3, 1.5/2, 0.5/1, -0.5/0, -1.5/-1, -2.5/-2, -3.5/-3} {
    \draw[-Stealth] (3.0, \y+0.2) -- (4.2, \y+0.2);
  }
...
\subsection{4. Delta-Sigma (\texorpdfstring{$\Delta\Sigma$}{Delta-Sigma}) ADC (Oversampling \& Noise Shaping)}
...
\begin{tikzpicture}[scale=0.95, transform shape]
  % Input
  \node[left] at (-1.5, 0) {$V_{in}$};
  \draw[thick] (-1.2, 0) to[short, o-*] (-0.8, 0);
  
  % Summer
  \node[draw=myamber, fill=hdramber, circle, minimum size=0.6cm, thick] (sum) at (0, 0) {$+$};
  \draw[-Stealth, thick] (-0.8, 0) -- (sum);
  
  % Integrator
  \node[draw=myteal, fill=hdrteal, minimum width=1.5cm, minimum height=0.9cm, font=\small, thick, align=center, text width=2.4cm] (int) at (2.2, 0) {Integrator\\$H(z) = \frac{z^{-1}}{1-z^{-1}}$};
  \draw[-Stealth, thick] (sum) -- (int);
  
  % Quantizer (ADC)
  \node[draw=myteal, fill=hdrteal, minimum width=1.4cm, minimum height=0.9cm, font=\small, thick, align=center, text width=2.2cm] (quant) at (4.8, 0) {Quantizer\\(1-Bit ADC)};
  \draw[-Stealth, thick] (int) -- (quant);
  
  % Output
  \draw[-Stealth, thick] (quant) -- (6.8, 0) node[right, font=\small] {Digital Output};
  \draw[thick] (6.1, 0) to[short, *-] (6.1, -1.5);
  
  % Feedback DAC
  \node[draw=mypurple, fill=hdrpurple, minimum width=1.4cm, minimum height=0.9cm, font=\small, thick, align=center, text width=2.2cm] (dac) at (3.5, -1.5) {1-Bit DAC};
  \draw[-Stealth, thick] (6.1, -1.5) -- (dac);
